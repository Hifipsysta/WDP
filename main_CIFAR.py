...
parser.add_argument('--batch_size_train', type=int, nargs='?', action = 'store', default=512, 
                    help='batchsize of data')

parser.add_argument('--batch_size_test', type=int, nargs='?', action = 'store', default=512, 
                    help='batchsize of data')

parser.add_argument('--n_epochs', type=int, nargs='?', action='store', default=60,
                    help='How many epochs to train. Default: 30.')
parser.add_argument('--learning_rate', type=float, nargs='?', action='store', default=0.5,
                    help='learning rate for model training.  Default: 1e-3.')

parser.add_argument('--scale', type=float, nargs='?', action='store', default =5e-4,
...
parser.add_argument('--device', type=str, nargs='?', action='store', default='cuda0')
        ...
        for batch_idx, (feature, target) in enumerate(train_loader):
            ...
            if accountant:
                with torch.no_grad():
                    ...
                    '''Moments Account after WDP and BDP'''
                    if len(accountant)>2:
