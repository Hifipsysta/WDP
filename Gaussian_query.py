...
def Gaussian_mechanism(scale, size, distribution_ = True):
    X = np.random.normal(loc=1, scale=scale, size=size)	
...
def get_different_scale_order_renyi_wasser(order_start, order_end, order_num, scale_list, size):
	...
	for i in range(len(scale_list)):
		for order in np.linspace(order_start, order_end, order_num):
			...
			X1,Y1 = Gaussian_mechanism(scale=scale_list[i], size=size, distribution_=True) 
			X2,Y2 = Gaussian_mechanism(scale=scale_list[i], size=size, distribution_=False)
...
def plot_different_scale_wasserstein(scale_start, scale_end, scale_num, size):
	...
	for scale in np.linspace(scale_start, scale_end, scale_num):
		...
		X1,Y1 = Gaussian_mechanism(scale=scale, size=size, distribution_=True) 
		X2,Y2 = Gaussian_mechanism(scale=scale, size=size, distribution_=False)
		#wasser_list.append(wasserstein_distance(X2, Y2))
		renyi_list.append(renyi_divergence(1, X1, Y1))
...
plot_different_scale_wasserstein(scale_start = 1, scale_end = 10, scale_num = 100, size = len(list(datacount)))
